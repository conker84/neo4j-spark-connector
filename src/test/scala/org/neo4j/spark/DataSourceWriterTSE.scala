...
import org.neo4j.driver.types.{IsoDuration, Point, Type}
...
case class Person(name: String, surname: String, age: Int, bornIn: Point2d, livesIn: Point3d)
...
class DataSourceWriterTSE extends SparkConnectorScalaBaseTSE {
  ...
  def `should write nodes with point-2d array values into Neo4j`(): Unit = {
    ...
    val records = SparkConnectorScalaSuiteIT.session().run(
      ...
      .map(r => {
        r.get("data").asList[Point]({ value: Value => value.asPoint() })
          .asScala
  ...
  def `should write nodes with duration array values into Neo4j`(): Unit = {
    ...
    val records = SparkConnectorScalaSuiteIT.session().run(
      """MATCH (p:BeanWithDuration)
        |RETURN p.data AS data
        |""".stripMargin).list().asScala
      .map(r => r.get("data").asList({ value: Value => value.asIsoDuration() }).asScala)
      .map(r => r.map((data: IsoDuration) => (data.months, data.days, data.seconds, data.nanoseconds)))
      .toSet
  ...
  def `should write nodes into Neo4j with points`(): Unit = {
    ...
    val ds = (1 to total)
      .map(i => Person(name = "Andrea " + i, "Santurbano " + i, rand.nextInt(100),
        Point2d(srid = 4326, x = 12.5811776, y = 41.9579492),
      ...
      """MATCH (p:Person:Customer)
        ...
        .filter(r => {
          ...
          (map.get("name").isInstanceOf[String]
            ...
            && map.get("bornIn").isInstanceOf[InternalPoint2D]
  ...
  def `should throw an error because the node already exists`(): Unit = {
    SparkConnectorScalaSuiteIT.session()
      .writeTransaction(t => t.run("CREATE CONSTRAINT ON (p:Person) ASSERT p.surname IS UNIQUE"))
    SparkConnectorScalaSuiteIT.session()
      .writeTransaction(t => t.run("CREATE (p:Person{name: 'Andrea', surname: 'Santurbano'})"))
    ...
    try {
      ...
      SparkConnectorScalaSuiteIT.session()
        .writeTransaction(t => t.run("DROP CONSTRAINT ON (p:Person) ASSERT p.surname IS UNIQUE"))
    }
  }
  ...
  def `should update the node that already exists`(): Unit = {
    SparkConnectorScalaSuiteIT.session()
      .writeTransaction(t => t.run("CREATE CONSTRAINT ON (p:Person) ASSERT p.surname IS UNIQUE"))
    SparkConnectorScalaSuiteIT.session()
      .writeTransaction(t => t.run("CREATE (p:Person{name: 'Federico', surname: 'Santurbano'})"))
