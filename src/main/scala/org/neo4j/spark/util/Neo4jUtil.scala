...
object Neo4jUtil {
  ...
  def convertFromNeo4j(value: Any): Any = value match {
    case d: InternalIsoDuration => {
      ...
      InternalRow.fromSeq(Seq("duration", months, days, seconds, nanoseconds, UTF8String.fromString(d.toString)))
    }
  ...
  def convertFromSpark(value: Any, schema: StructField = null): AnyRef = value match {
    ...
    case struct: GenericRowWithSchema => {
      ...
      struct.getAs[UTF8String]("type").toString match {
        case "point-2d" => Values.point(struct.getAs[Number]("srid").intValue(),
          struct.getAs[Number]("x").doubleValue(),
          struct.getAs[Number]("y").doubleValue())
        case "point-3d" => Values.point(struct.getAs[Number]("srid").intValue(),
          struct.getAs[Number]("x").doubleValue(),
          struct.getAs[Number]("y").doubleValue(),
          struct.getAs[Number]("z").doubleValue())
        case "duration" => Values.isoDuration(struct.getAs[Number]("months").longValue(),
          struct.getAs[Number]("days").longValue(),
          struct.getAs[Number]("seconds").longValue(),
          struct.getAs[Number]("nanoseconds").intValue())
        case "offsetTime" | "time" => Values.value(OffsetTime.parse(struct.getAs[String]("value")))
        case "localTime" => Values.value(LocalTime.parse(struct.getAs[String]("value")))
